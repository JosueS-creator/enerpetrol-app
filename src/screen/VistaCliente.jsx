...
export default function VistaCliente({ usuario }) {
  ...
  async function handleEnviar() {
    ...
    // 1. Subir la imagen real al almacenamiento de Supabase
    ...
    // 2. Crear el registro de la factura
  ...
  return (
    <div className="px-5 pt-2 pb-6">
      ...
      <div className="mt-6 rounded-xl border p-5" style={{ borderColor: BORDER, background: CARD }}>
        ...
        <p className="text-center text-xs mt-3" style={{ color: TEXT_MUTED }}>Consumo acumulado este período</p>
      </div>
      ...
      <div className="mt-4 rounded-xl border p-4 flex items-center gap-3" style={{ borderColor: `${GREEN}50`, background: `${GREEN}0D` }}>
        ...
        <p className="text-xs text-right" style={{ color: TEXT_MUTED, maxWidth: 90 }}>1 punto<br />por galón</p>
      </div>
      ...
      {perfil.galones_acumulados >= UMBRAL_PUNTOS_CANJE && (
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-white/85">Acércate a tu gasolinera con tu código de la tarjeta.</p>
          </div>
        </div>
      )}
      ...
      <div className="mt-6">
        ...
        <div className="rounded-xl border border-dashed p-4" style={{ borderColor: '#C7CFD6', background: CARD }}>
          ...
          <button
            ...
            className="w-full rounded-lg py-2.5 text-sm font-semibold flex items-center justify-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed text-white"
            style={{ background: GREEN }}
          >
            <Upload size={15} /> {subiendo ? 'Subiendo...' : 'Enviar para revisión'}
          </button>
          {enviado && (
            <p className="text-xs text-center mt-2.5" style={{ color: '#4A9123' }}>Factura enviada. Será revisada por el equipo.</p>
          )}
        </div>
      </div>
