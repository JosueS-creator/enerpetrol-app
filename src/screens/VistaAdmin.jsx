...
export default function VistaAdmin() {
  ...
  async function resolver(facturaId, clienteId, galonesOriginales, nuevoEstado, razon) {
    ...
    if (nuevoEstado === 'aprobada') {
      const { data: facturaInfo } = await supabase.from('facturas').select('estacion_id').eq('id', facturaId).single()
      let acumulaPuntos = true
      if (facturaInfo?.estacion_id) {
        const { data: est } = await supabase.from('estaciones').select('acumula_puntos').eq('id', facturaInfo.estacion_id).single()
  ...
  async function cambiarEstado(factura, nuevoEstado) {
    ...
    if (nuevoEstado === 'aprobada') {
      let acumulaPuntos = true
      if (factura.estacion_id) {
        const { data: est } = await supabase.from('estaciones').select('acumula_puntos').eq('id', factura.estacion_id).single()
        acumulaPuntos = est?.acumula_puntos !== false
      }
      if (acumulaPuntos) {
